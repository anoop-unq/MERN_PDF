...
const PostList = ({ posts: postsProp,id:userId  }) => {
  ...
  const [filter, setFilter] = useState('all'); // 'all', 'pdf', 'text'
  ...
  const filteredPosts = posts.filter(post => {
    const hasPdf = post.fileUrl && post.fileUrl.trim() !== '';
    
    if (filter === 'pdf') return hasPdf;
    if (filter === 'text') return !hasPdf;
      ...
      {isFiltering ? (
        ...
        <div className="max-w-md mx-auto text-center py-20">
          ...
          <h3 className="text-2xl font-bold bg-gradient-to-r from-purple-600 to-pink-600 bg-clip-text text-transparent mb-3">
            {filter === 'pdf' ? 'No resume posts yet' : 
             filter === 'text' ? 'No text posts yet' : 'No posts yet'}
          </h3>
          <p className="text-gray-600 text-lg">
            {filter === 'pdf' ? 'Start sharing your amazing resumes!' : 
                        ...
                        <div className="flex items-center space-x-4">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <a 
                              ...
                              className="mt-2 inline-block px-4 py-2 bg-blue-500 hover:bg-blue-600 text-white text-xs font-semibold rounded-lg transition-colors"
                            >
                              View Resume
                            </a>
                          </div>
