...
export const createPost = async (req, res) => {
  let pdfData = null; // Declare pdfData at the top level of the function
  
  try {

      console.log('Request received - User ID:', req.userId);
    ...
    if (!hasContent && !hasPdf) {
      return res.status(400).json({
        success: false,
        error: 'Post must contain either content or a PDF'
      });
    }
    ...
    const populatedPost = await Post.findById(post._id)
      .populate({
        path: 'author',
        select: 'name username avatar photo'
      })
      .lean();
    ...
    const responseData = {
      ...
      fileUrl: populatedPost.fileUrl,
      filePublicId: populatedPost.filePublicId,
      fileType: populatedPost.fileType,
      fileUuid: populatedPost.fileUuid,
      fileName : populatedPost.fileName,
      author: {
    ...
    if (pdfData && pdfData.publicId) {
      try {
        await cloudinary.uploader.destroy(pdfData.publicId, {
          resource_type: 'raw'
        });
      } catch (cleanupError) {
...
export const updatePost = async (req, res) => {
  try {
    ...
    // Extract content from req.body - handle both FormData and JSON
    const content = req.body.content;
    console.log(content,"7285")
    // At least one of content or PDF must be present
    if ((!content || !content.trim()) && !req.file) {
      return res.status(400).json({
        success: false,
        error: 'Post must contain either content or a PDF file'
      });
    }
    ...
    // Prepare update data - handle case where content might be empty
    const updateData = {
      content: content ,
      ...(pdfData && {
        fileUrl: pdfData.url,
    ...
    // If no new file and no content, keep existing values
    if (!req.file && (!content || !content.trim())) {
      updateData.content = post.content;
      updateData.fileUrl = post.fileUrl;
      updateData.filePublicId = post.filePublicId;
      updateData.fileType = post.fileType;
      updateData.fileUuid = post.fileUuid;
    }

    // Remove PDF if requested - check both req.body and req.query
    const removeFile = req.body.removeFile === 'true' || req.query.removeFile === 'true';
    if (removeFile) {
      if (post.filePublicId) {
        await cloudinary.uploader.destroy(post.filePublicId, {
          resource_type: 'raw'
